...
def initiation():
    global termination_time, random_seed, instance_name, vertices_num, depot_vertex_id, required_edges_num, non_required_edges_num, vehicles_num, capacity, total_cost_of_required_edges, distance_matrix
    console = sys.argv
...
# # add the finished-task into the formatted string 'arc'
# def add_arc(arc, idx_1, idx_2):
#     arc += ',(' + str(idx_1 + 1) + ',' + str(idx_2 + 1) + ')'
#     return arc


# # just for usability test
# # finish one task and back to depot
# def brute_force():
#     arcs = 's '
#     quality = 0
#
...
#         idx_1, idx_2 = task
#         arc = '0'
#         min_dist_1 = distance_matrix[depot_vertex_id][idx_1] + distance_matrix[idx_2][depot_vertex_id]
#         min_dist_2 = distance_matrix[depot_vertex_id][idx_2] + distance_matrix[idx_1][depot_vertex_id]
#         if min_dist_1 < min_dist_2:
#             quality += min_dist_1 + cost_map[(idx_1, idx_2)]
#             arc = add_arc(arc, idx_1, idx_2)
#         else:
#             quality += min_dist_2 + cost_map[(idx_2, idx_1)]
#             arc = add_arc(arc, idx_2, idx_1)
#
#         completed_task_list.append((idx_1, idx_2))
#         completed_task_list.append((idx_2, idx_1))
#         arc += ',0,'
#         arcs += arc
#
#     arcs = arcs.removesuffix(',')
#     return arcs, quality
...
def path_scanning():
    ...
    while task_list:
        ...
        # Do the best task
            ...
            task_list.remove((best_task_dst_vertex, best_task_src_vertex))
...
def flip(total_quality):
    for j in range(len(routes)):
        ...
        for i in range(len(route)):
            ...
            cost_before_change = distance_matrix[prev_arc_end][now_arc_start] + distance_matrix[now_arc_end][next_arc_start]
            cost_after_change = distance_matrix[prev_arc_end][now_arc_end] + distance_matrix[now_arc_start][next_arc_start]
...
def find_in_route(cur_arc_idx, num, cur_route, cur_route_quality, cur_route_idx):
    ...
    for i in range(1, len(cur_route)):
        if i == cur_arc_idx or i == cur_arc_idx + num or i == cur_arc_idx + num + 1:
            continue
        new_prev_arc_end = (cur_route[i - 1])[1]
        new_next_arc_start = (cur_route[i])[0]
        ...
        if cur_route_quality - new_cost > most_decrease_delta:
            ...
            target_route_idx = cur_route_idx

    return most_decrease_delta, target_route_idx, target_arc_idx, target_route_cost


def find_across_routes(cur_arc_start, cur_arc_end, cur_arc_demand, cur_inside_cost, cur_route_cost, cur_route_quality,
                       cur_route_idx):
    target_route_idx = None
    target_arc_idx = None
    target_route_cost = None
    ...
    for i in range(len(routes)):
        ...
        if new_route_load + cur_arc_demand > capacity or cur_route_idx == i:
            continue
        for j in range(1, len(new_route)):
            new_prev_arc = new_route[j - 1]
            new_next_arc = new_route[j]
            new_prev_arc_end = new_prev_arc[1]
            new_next_arc_start = new_next_arc[0]
            ...
            if (cur_route_quality + new_route_quality) - (cur_route_cost + new_route_cost) > most_decrease_delta:
                most_decrease_delta = (cur_route_quality + new_route_quality) - (cur_route_cost + new_route_cost)
                target_route_idx = i
                target_arc_idx = j
...
def single_arc_insertion(cur_route_idx, cur_arc_idx):
    next_route = False
    ...
    delta_in_route, target_route_idx_in_route, target_arc_idx_in_route, target_route_cost_in_route \
        = find_in_route(cur_arc_idx, 0, cur_route, cur_route_quality, cur_route_idx)

    delta_across_route, target_route_idx_across_route, target_arc_idx_across_route, target_route_cost_across_route \
        = find_across_routes(cur_arc_start, cur_arc_end, cur_arc_demand, cur_inside_cost, cur_route_cost,
                             cur_route_quality, cur_route_idx)

    # find a better place
    if delta_in_route > delta_across_route:
        most_delta = delta_in_route
        target_arc_idx = target_arc_idx_in_route
        target_route_idx = target_route_idx_in_route
        target_route_cost = target_route_cost_in_route
        origin_route_cost = target_route_cost_in_route
        addition_load = 0
    else:
        most_delta = delta_across_route
        target_arc_idx = target_arc_idx_across_route
        target_route_idx = target_route_idx_across_route
        target_route_cost = target_route_cost_across_route
    ...
    return most_delta, next_route


def double_arc_insertion(cur_route_idx, cur_arc_idx):
    next_route = False
    cur_route_info = routes[cur_route_idx]
    cur_route, cur_route_quality, cur_route_load = cur_route_info
    prev_arc_end = (cur_route[cur_arc_idx - 1])[1]
    ...
    delta_in_route, target_route_idx_in_route, target_arc_idx_in_route, target_route_cost_in_route \
        = find_in_route(cur_arc_idx, 1, cur_route, cur_route_quality, cur_route_idx)
...
def single_insertion(total_quality):
    i = 0
    while i < len(routes):
        cur_route, cur_route_quality, cur_route_load = routes[i]
        j = 1
        next_route = False
        while j < len(cur_route) - 1:
            delta, next_route = single_arc_insertion(i, j)
            total_quality -= delta
            if next_route:
                break
            j += 1
...
def double_insertion(total_quality):
    i = 0
    while i < len(routes):
        cur_route, cur_route_quality, cur_route_load = routes[i]
        j = 1
        next_route = False
        while j < len(cur_route) - 2:
            delta, next_route = double_arc_insertion(i, j)
            total_quality -= delta
            if next_route:
                break
            j += 1
...
def arc_swap_across_routes(cur_route_idx, cur_arc_idx):
    target_route_idx = None
    target_arc_idx = None
    target_route_cost = None
...
def swap(total_quality):
    for i in range(len(routes)):
        cur_route, cur_route_quality, cur_route_load = routes[i]
        for j in range(1, len(cur_route) - 1):
            delta = arc_swap_across_routes(i, j)
            total_quality -= delta
...
if __name__ == '__main__':
    ...
    quality = path_scanning()

    for i in range(50000):
        quality = flip(quality)
    # print(arcs_to_string())
    # print(quality)
        quality = single_insertion(quality)
    # print(arcs_to_string())
    # print(quality)
        quality = double_insertion(quality)
    # print(arcs_to_string())
    # print(quality)
        quality = swap(quality)

    s_line = arcs_to_string()
    quality_line = 'q ' + str(quality)
    output_info = s_line + '\n' + quality_line
    print(output_info)
